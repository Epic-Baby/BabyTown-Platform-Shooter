...
const BoomOfDoom = preload("res://Weapons//Bullets//BoomOfDoom.tscn")
...
func _process(_delta):
	...
	if get_viewport_rect().size.x/2 > get_viewport().get_mouse_position().x:
		...
		$Sprite.set("position", Vector2(Position.x, Position.y-56))
		...
		look_at(MousePosition - Vector2(0,28))
	else:
		$Sprite.set("scale", Vector2(1,1))
